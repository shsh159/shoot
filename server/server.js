...
app.post('/add', (req, res) => {
  ...
  db.getConnection((err, connection) => {
    ...
    const query =
      'INSERT INTO history (writer, type, amount, description, target_date, create_date_time) VALUES (?, ?, ?, ?, ?, NOW())';
    connection.query(
      query,
      [writer, type, amount, description, date],
      (err, result) => {
        connection.release();
        if (err) {
          console.log('query error', err);
...
app.put('/update', (req, res) => {
  ...
  db.getConnection((err, connection) => {
    ...
    const query =
      'UPDATE history SET writer = ?, type = ?, amount = ?, description = ?, target_date = ?, update_date_time = NOW() WHERE id = ?';
    connection.query(
      query,
      [writer, type, amount, description, date, id],
      (err, result) => {
        connection.release();
        if (err) {
          console.log('query error', err);
